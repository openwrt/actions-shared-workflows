...
sub-agent / Task tool to spawn isolated reviewers. Local `git` commands
...
2. **Spawn sub-agents in parallel.** For each PR number in `pr_numbers`,
   spawn one sub-agent via the Task / Agent tool. Issue all spawn calls
   ...
   review comment. The parent's only tool calls are the sub-agent
   spawn calls.

   Sub-agent prompt template — substitute the three placeholders, then
   pass the resulting string as the sub-agent's prompt:
   ...
   │                                                                  │
   │ 5. **Decide which extras to clone.** `<EXTRA_REPOS>` is a        │
   │    comma-separated list of `owner/name:ref` or full              │
   │    `http(s)://...:ref` reference repositories — *available* for  │
   │    cloning, not a list to clone unconditionally.                 │
   │                                                                  │
   │    Inspect this PR's changed file paths. Lean toward cloning     │
   │    an entry when the diff is materially about code the entry    │
   │    grounds (kernel internals → kernel ref, U-Boot → U-Boot ref,  │
   │    `package/network/services/dnsmasq/` → dnsmasq, etc.). Skip    │
   │    only when the diff is clearly unrelated.                      │
   │                                                                  │
   │    For each entry you decide to clone, split on the **last**     │
   │    `:` — left side is the repository (either `owner/name` for    │
   │    GitHub or a full http(s) URL), right side is the ref.         │
   │    Clone into a sub-agent-private subdirectory to avoid races    │
   │    with other sub-agents:                                        │
   │                                                                  │
   │        # GitHub shorthand                                        │
   │        git clone --depth=1 --branch <ref> \                      │
   │          https://github.com/<owner>/<name> \                     │
   │          ~/extra/pr-<NUM>/<name>-<ref>                           │
   │                                                                  │
   │        # Full URL                                                │
   │        git clone --depth=1 --branch <ref> \                      │
   │          <url> ~/extra/pr-<NUM>/<short>-<ref>                    │
   │                                                                  │
   │    For full URLs, derive `<short>` from the last path component  │
   │    with any trailing `.git` stripped (e.g. `dnsmasq.git` →       │
   │    `dnsmasq`). Use clones as read-only reference. Never modify.  │
   │                                                                  │
   ...
   │    the maintainer can judge. Don't post pure speculation with    │
   │    no evidence anchor.                                           │
   │                                                                  │
