...
| `extra_repos` | `''`             | Comma-separated `owner/name:ref`. Each entry must include the tag or branch to check out (e.g. `gregkh/linux:v6.18.21`). The routine inspects the PR and only clones the entries it actually needs. |
...
Each entry is `owner/name:ref` (the ref is required and is checked out as a
shallow clone). The routine itself decides whether each entry is needed
based on the PR's changed files, so it is safe to list reference repos you
only sometimes need (for example, the Linux stable tree at a specific
kernel version). The routine will skip cloning when the PR doesn't touch
anything that needs cross-repo context.
